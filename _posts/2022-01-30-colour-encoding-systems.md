...
You may even have heard that the primary colours are cyan, magenta, and yellow (CMY), and these are true primary colours, in that you can get every other colour using them, but they are subtractive primary colours, meaning they apply when using inks or dyes that absorb certain colours of light. This is why printers use CMY (well, printers really use CMYK where K is black, but this is because black is a common printing colour, and the alternative is to run over all black areas with cyan, magenta, and yellow).

What we are interested in are additive primary colours, which are what you would need if you wanted to shine lights of different colours in the same spot on a wall to get the different colours. These are the colours that are emitted by pixels, these are colours used when specifying colours in code, and they are the colours that stimulate our eyes' colour-sensing cells.
...
Hex codes are written with a "#" followed by three two-digit numbers in hexadecimal representing the amounts red, green, and blue respectively. Let's break that down.
...
The values in each argument numbers out of 255 (which is the same as two hexadecimal digits in a hex code).
...
This will be highly dependent on the environment you are coding in.
